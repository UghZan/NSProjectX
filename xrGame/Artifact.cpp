...
void CArtefact::Load(LPCSTR section)
{
...

	{
		...
		m_fPsyRestoreSpeed = pSettings->r_float(section, "psy_health_restore_speed");
...
float CArtefact::GetStatWithVariation(AFProperty stat_name)
{
	...
	{
	case aHealthRestore:
		return m_fHealthRestoreSpeed * coeff;
	case aPsyRestore:
		return m_fPsyRestoreSpeed * coeff;
	case aRadRestore:
		return m_fRadiationRestoreSpeed * coeff;
	case aSatietyRestore:
		return m_fSatietyRestoreSpeed * coeff;
	case aPowerRestore:
		return m_fPowerRestoreSpeed * coeff;
	case aBleedingRestore:
		return m_fBleedingRestoreSpeed * coeff;
	case aInventoryWeight:
		return m_fAdditionalInvWeight * coeff;
	case aInventoryWeight2:
		return m_fAdditionalInvWeight2 * coeff;
	case aBurnProtection:
		return (1.0f - m_ArtefactHitImmunities.GetHitImmunity(ALife::eHitTypeBurn)) * coeff;
	case aStrikeProtection:
		return (1.0f - m_ArtefactHitImmunities.GetHitImmunity(ALife::eHitTypeStrike)) * coeff;
	case aShockProtection:
		return (1.0f - m_ArtefactHitImmunities.GetHitImmunity(ALife::eHitTypeShock)) * coeff;
	case aWoundProtection:
		return (1.0f - m_ArtefactHitImmunities.GetHitImmunity(ALife::eHitTypeWound)) * coeff;
	case aRadProtection:
		return (1.0f - m_ArtefactHitImmunities.GetHitImmunity(ALife::eHitTypeRadiation)) * coeff;
	case aTelepaticProtection:
		return (1.0f - m_ArtefactHitImmunities.GetHitImmunity(ALife::eHitTypeTelepatic)) * coeff;
	case aChemBurnProtection:
		return (1.0f - m_ArtefactHitImmunities.GetHitImmunity(ALife::eHitTypeChemicalBurn)) * coeff;
	case aExplosionProtection:
		return (1.0f - m_ArtefactHitImmunities.GetHitImmunity(ALife::eHitTypeExplosion)) * coeff;
	case aFireWoundProtection:
		return (1.0f - m_ArtefactHitImmunities.GetHitImmunity(ALife::eHitTypeFireWound)) * coeff;
	}
}
