...
void CLight_Compute_XFORM_and_VIS::compute_xf_spot(light* L)
{
	...
	L->X.S.project.build_projection(_min(L->cone + deg2rad(5.f), PI * 0.98f), 1.f, /*SMAP_near_plane*/(L->virtual_size > MIN_VIRTUAL_SIZE) ? L->virtual_size : MIN_VIRTUAL_SIZE, L->range + EPS_S);
